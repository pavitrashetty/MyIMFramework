/*
MOTIVE:
  Given product images, create a vertical ad video. Pseudo trail 3.
*/
...
int main(int argc,char **argv)
{
  ...
  Image *text5 = new Image ("/home/pavitra-ubuntu/Downloads/inputs/text_page5.jpg");
  ...
  mvobj->set_background(temp_fm);
  prod_mfm = new Multiframe_Manager();    //bad bad girl
  prod_mfm->add_frame_manager(mvobj->draw_line(new Position(0,1010), new Position(150,1010), 1));
  prod_mfm->add_frame_manager(ftrans->enlarge(text5, new Resolution(0,0), new Resolution(300,100), 2));
  prod_mfm->get_frame_manager(0)->dump_frames("new_frames","fm1_","jpg",1);
  prod_mfm->get_frame_manager(1)->dump_frames("new_frames","fm2_","jpg",1);
  prod_mfm->composite_frames("simple_overlay", new Position(100, 1100), 2);
  main_mfm->add_frame_manager(prod_mfm->get_first_frame_manager());
